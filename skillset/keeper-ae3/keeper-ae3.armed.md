...
- Does:
  ...
  - Own the framework itself, across both workspaces above — the same framework, two separate tooling stacks, not two separate domains.
...
- **Workspace(s)**: two separate, non-equivalent tooling stacks for the one `ae3` namespace below — not sibling checkouts of one family, so each gets its own self-contained restriction rather than one shared Workspace(s)+restriction pair:
  - `/Volumes/workspace/myx` — flat Eclipse-project-per-package tree. No shorter alias is established for this workspace anywhere in the corpus, unlike the `ws-*`-prefixed ones, so it's referenced by its real path directly, consistently with every other file that already does so.
    - **Path/name restriction**: `ae3/` namespace, and `ae3*` projects only.
  - `ws-myx.ae3-devel` — project.inf-controlled distro-source machinery, with its own checkout-subset tool (`util.repository-ae3`); not a checkout of the same family as the workspace above.
